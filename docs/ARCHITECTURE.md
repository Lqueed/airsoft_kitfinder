...
- **Playwright не используем**: проверено, что каталоги магазинов рендерятся
  server-side (товары есть в статичном HTML) — `httpx + selectolax` легче, быстрее
  и надёжнее для крона. Браузер понадобился бы только для JS-SPA — не наш случай.
- **Первый реализованный магазин — strikeplanet.ru** (1C-Bitrix). Парсер обходит
  дерево каталога от корня (BFS/DFS с дедупом URL), пагинация через `?PAGEN_1=`.
  `external_id` = Bitrix `data-element-id`, наличие — по кнопке «в корзину»
  (`.cart--add`; её отсутствие + «Сообщить о поступлении» = нет в наличии).
  `ingest_shop(..., limit=N)` — smoke-прогон/защитный кап (деактивацию пропускает).
