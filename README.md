...
- список и редактор китов (позиции fixed/flexible, курация вариантов, публикация,
  загрузка до 10 фото в S3 с выбором обложки);
- **товары** (`/admin/products`): список канонических товаров с агрегатами (офферы,
  магазины, вилка цены), поиск/фильтры/сортировка, правка полей, ручной матчинг
  (объединение дублей, привязка/отвязка офферов, создание товара из оффера) и
  массовые действия по чекбоксам (объединить/категория/бренд/удалить);
- каталог магазинов: поиск по сырым офферам, ручная привязка и создание товара.
